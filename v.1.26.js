...
$(async function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                if (userDb.survey_answers) {
                    ...
                    if (Number(survey.progress) === 100) $(`.get-started--checkbox`).eq(2).addClass('filled')
    ...
    async function handleUpload(files, category) {
        parsedCategory = toSnakeCase(category);
        const filenames = Array.from(files).map(file => file.name);
        const url = `https://server.xncaptive.com/api/users/documents/${parsedCategory}/upload-urls`;
    ...
    async function handleGetDocuments() {
        const url = `https://server.xncaptive.com/api/users/documents/download-urls`
    ...
    //select files
    $('.upload--browse').click(function (e) {
    ...
    $dropArea.on('drop', function (e) {
        ...
        if (files.length) {
            processFiles(files);  // A function to process files
        }
    });

    function processFiles(files) {
        for (var i = 0; i < files.length; i++) {
            filesList.push(files[i]);  // Assuming filesList is already defined and used in your upload logic
        }
        updateCard();  // Update the UI with the file information
    }
