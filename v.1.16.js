...
$(async function () {
    ...
    const getUserInfo = async (username) => {
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                // NEEED TO CONNECT SURVEY ANSWERS TO BACKEND...?  if (!userDb.survey_answers) { $('#tab--upload').addClass('hidden') }
    ...
    async function handleGetDocuments() {
        ...
        responseData.forEach((element) => {
            const sameCategoryItems = responseData.filter(item => item.category === element.category);
